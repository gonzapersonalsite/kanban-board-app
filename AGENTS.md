...
## State Management
- <chosen-state-lib> is the ONLY state library. Do NOT introduce other tools
  for cross-component state.
- Store files live at: <layer>/<slice>/model/store.ts
- Stores must follow the <specific-pattern> pattern.

## Routing
- react-router with lazy loading is the routing standard.
...
- You **MUST NOT** modify this file or `.agents/skills/react-fsd-maintainer/SKILL.md`.
