...
describe('dndSlice', () => {
  ...
  describe('reorderColumns', () => {
    ...
    it('ignores_reorder_when_source_index_is_out_of_bounds', () => {
      const initial = store.getState().columnsByBoard[BOARD_MAIN_ID]

      store.getState().reorderColumns(-1, 1)

      expect(store.getState().columnsByBoard[BOARD_MAIN_ID]).toEqual(initial)
    })
  })
  ...
  describe('moveTask', () => {
    it('moves_a_task_to_another_column_at_the_requested_index', () => {
      ...
      const tasks = store.getState().tasksByBoard[BOARD_MAIN_ID]
    ...
    it('does_not_mutate_state_when_task_id_is_missing', () => {
      const before = createKanbanFixture().tasksByBoard[BOARD_MAIN_ID]

      store.getState().moveTask(COLUMN_TODO_ID, COLUMN_DONE_ID, 'missing-task', 0)

      expect(store.getState().tasksByBoard[BOARD_MAIN_ID]).toEqual(before)
    })
  })
})
